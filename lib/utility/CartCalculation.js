...
const getConditonalValue = async (conditions, currentPrice = 0) => {
	...
	itemTotal = itemTotal + variations;
...
const calculateCartSubTotal = async items => {
	...
	for (const item of items) {
		const { conditions = [], price, multiplier } = item;
		if (!item._itemId) {
			item['_itemId'] = uuid();
		}
		const { itemTotal, itemGlobals } = await getConditonalValue(conditions, price * multiplier);

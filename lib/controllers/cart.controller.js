...
const updateCart = async (content, globalConditions, userId) => {
	try {
		saveCartValidator(content, globalConditions, userId, true);
		const data = await composePayload(content, globalConditions, userId);
		const [updated] = await updateCartDAO(data, userId);
		const isSuccess = !!updated;
		if (!isSuccess) {
			throw new Error(failureMessage.INVALID_USER_ID);
		}
		return data;
	} catch (err) {
		throw new Error(err);
...
const deleteCart = async userId => {
	try {
		...
		const isDeleted = !!(await deleteCartDAO(userId));
		if (!isDeleted) {
			throw new Error(failureMessage.INVALID_USER_ID);
		}
		return true;
...
const composePayload = async (content, /*NOSONAR */ globalConditions = [], userId) => {
	...
	const combinedGlobalConditions = [...itemGlobalConditions, ...globalConditions].sort(
		(a, b) =>

...
const updateItemQuantity = async (userId, _itemId, quantity) => {
	try {
		updateQuantityValidator(userId, _itemId, quantity);
		...
		// When Item Quantity is being set to Zero and Cart has only One item
		if (quantity === 0 && cart.content.length === 1) {
			await deleteCartDAO(userId);
			return;
		}

		/* When incoming quantity is greater than zero then it is updated accordingly or else
		the concerned item is removed from cart*/
		if (quantity > 0) {
			cart.content[concernedItemIndex].quantity = quantity;
		} else {
...
const composePayload = async (content, globalConditions, userId) => {
